    ...
    return (
        <Box sx={{ mt: 2 }}>
            <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2, px: 1 }}>
                <Typography variant="h6" sx={{ fontSize: '1.1rem', fontWeight: 'bold' }}>댓글</Typography>
                <Typography variant="h6" sx={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#03c75a' }}>{commentResp?.data?.totalCount}</Typography>
            </Stack>

            <Box sx={{ borderTop: '1px solid #333', borderBottom: '1px solid #e0e0e0', mb: 3 }}>
                {comments.length === 0 ? (
                    ...
                    comments.map(comment => {
                        ...
                        // 살아있는 자식이 있는지 확인
                        const hasLiveChild = commentList.some(child => !child.isDeleted);
