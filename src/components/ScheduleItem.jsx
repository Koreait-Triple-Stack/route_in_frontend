...
const ScheduleItem = ({ day, routines, active, onReset, onUpdate, onToggle }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [activitiesValue, setActivitiesValue] = useState("");

  const handleEditClick = () => {
    const text = routines ? routines.map(a => a.exercise).join(", ") : "";
  ...
  const handleSaveClick = () => {
    if (onUpdate) {
      const newTexts = activitiesValue.split(",").map(item => item.trim()).filter(Boolean);
      onUpdate(newTexts);
    }
    setIsEditing(false);
  };
    ...
    <Paper variant="outlined" sx={{ p: 2, bgcolor: active ? '#f0f7ff' : 'white' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', minHeight: 80 }} >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'start', width: 380, alignItems: 'center', flexWrap: 'wrap' }} > 
          {isEditing ? (
            <TextField 
              ...
              placeholder="예: 스쿼트, 런지"
            />
          ) : (
            routines?.length !== 0 && routines?.map((act, index) => (
              <Typography 
                key={index} 
                variant="caption" 
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
          {isEditing ? (
            <>
              <Button size="small" variant="contained" onClick={handleSaveClick}>저장</Button>
              <Button size="small" color="inherit" onClick={() => setIsEditing(false)}>취소</Button>
            </>
          ) : (
            <>
              <Button size="small" onClick={handleEditClick}>수정</Button>
